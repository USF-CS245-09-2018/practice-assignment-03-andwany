...
public class Practice03Test {
	...
	/**
	 * print_array: prints the array of doubles... formatted so it fits ... on many
	 * small screens.
	 */
	...
	public int find_min_recursive() {
		return find_min_recursive(0);
	}

	public int find_min_recursive(int index) {
		if (index == arr.length - 1) {
			return index;
		}
		if (arr[find_min_recursive(index + 1)] < arr[index]) {
			return find_min_recursive(index + 1);
		}
		return index;
	}

	/**
	 * print_min: determines the min iteratively and recursively. ... and prints
	 * them both.
	 */
